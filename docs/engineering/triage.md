...
**The agent tried to apply `ready-for-agent` and the tracker said the label doesn't exist.**
Known open bug ([#616](https://github.com/mattpocock/skills/issues/616)). `setup-matt-pocock-skills` writes the label vocabulary into `docs/agents/triage-labels.md`, but does not create labels in trackers that require pre-created labels. Create the five state labels and two category labels yourself, once, with the tracker's CLI or UI, and it stops. Azure Boards is different: its template stores the roles as tags, which are created when applied. There is a community fix branch linked from the issue that hasn't been merged.
